...
module.exports = function startDefaultMetrics(globalStats, config) {
  ...
  const openCensusMetrics = new OpenCensusMetrics(normalizedConfig.stats || globalStats);
  ...
  const initialisedMetrics = metricsList.map(metric => {
    ...
    /* if (!init) {
      defaultMetric.metricNames.map(globalRegistry.removeSingleMetric, globalRegistry);
    }*/
